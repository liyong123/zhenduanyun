...
function basicInfo() {
    var quest = {
        questionRollCompanyId: localStorage.getItem('quesID')
    }
    app.DataRequest(URL + 'questionRoll/getEvaluatinMsg', quest, function (err) {}, function (data) {
        ...
        $('#renderPdf').attr('data-companyName', get(data, '[0].company.name', ''))
    })
}
...
function sub_quest() {
    var quest = {
        questionRollId: localStorage.getItem('questionRollId')
    }
    app.DataRequest(URL + 'count/allLevel', quest, function (err) {}, function (data) {
        if (!data[0]) {
            return
        }
        if (data[0].status == 1) {
            $('#headline').html(data[0].list[0].questionRollName.split('（')[1])

            var textHtml = ''
            var kind = data[0].list
            for (var i = 0; i < kind.length; i++) {
                ...
                textHtml += '<h4 class = "mes_type"> ' + kind[i].kindName.split('-')[2] + '能力 </h4>'
...
function sub_industry() {
    ...
    app.DataRequest(URL + 'overallEvaluation/getOverallEvaluationRealTime', quest, function (err) {}, function (data) {
        // 行业-所在位置
        var indFx = data[0].fx.industryZbAll
        var indFxText = ''
        var divID = new Array()
...
function radarDataL() {
    ...
    app.DataRequest(URL + 'count/getAreaScoreAll', quest, function (err) {}, function (data) {
        if (data[0].status != '1') {
            alert('数据请求失败，请重试')
            return
        }
