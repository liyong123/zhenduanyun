    ...
    Paging.prototype = {
        ...
        createDom: function () {
            var that = this,
                ...
                wrapLength = 0;
            totalPages > 2 ? wrapLength = 2 * liWidth : wrapLength = totalPages * liWidth;
            for (var i = 1; i <= that.options.totalPages - 1; i++) {
                i != 1 ? ulDom += '<li>' + i + '</li>' : ulDom += '<li class="sel-page">' + i + '</li>';
            }
            content = '<button type="button" id="prePage"  class="turnPage first-page">上一页</button>' +
                '<button class="turnPage" >&lt;</button>' +
                '<div class="pageWrap" style="width:' + wrapLength + 'px">' +
                '<ul id="pageSelect" style="transition:all ' + that.options.slideSpeed + 'ms">' +
                ulDom +
                '</ul></div>' +
                '<button class="turnPage" >&gt;</button>' +
                '<button type="button" id="nextPage" class="last-page">下一页</button>';
        ...
        bindEvents: function () {
            ...
            nextPage.on('click', function () {
                pageIndex++;
                if (pageIndex > lis.length + 1) {
                    pageIndex = lis.length + 1;
                }
